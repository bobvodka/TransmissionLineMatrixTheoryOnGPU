...
matrix44::matrix44(const float* array) {
	col[0].set(array[0], array[1], array[2], array[3]);
	col[1].set(array[4], array[5], array[6], array[7]);
	col[2].set(array[8], array[9], array[10], array[11]);
	col[3].set(array[12], array[13], array[14], array[15]);
}

void matrix44::getMatrix(float *m)
{
	m[0] = col[0].x;
	m[1] = col[0].y;
	m[2] = col[0].z;
