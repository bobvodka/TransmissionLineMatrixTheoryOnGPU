...
GLSLShader::GLSLShader(const std::string &filename, const GLenum shaderType) : compiled_(false)
{
	loadShader(filename,shaderType);
}
GLSLShader::GLSLShader(const GLenum shaderType)
{
...
void GLSLShader::setShaderSource(const std::string &code)
{
	const char *source = code.c_str();
	glShaderSource(handle_,1,static_cast<const GLchar**>(&source),NULL);
}

void GLSLShader::loadShader(const std::string &filename, const GLenum shaderType /* = GL_VERTEX_SHADER */)
{
	if(shaderType == GL_VERTEX_SHADER || shaderType == GL_FRAGMENT_SHADER)
		handle_ = glCreateShader(shaderType);
	else
...
char *GLSLShader::readShader(const std::string &filename)
{
  ...
  if (!temp.good())
  {
	  throw std::runtime_error("GLSLShader::readShader - Error opening shader file");
  }
  int count = 0;
  char *buf;
