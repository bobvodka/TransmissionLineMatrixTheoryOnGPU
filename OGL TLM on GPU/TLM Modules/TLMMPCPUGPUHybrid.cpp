...
namespace TLM
{
	TLMMPCPUGPUHybrid::TLMMPCPUGPUHybrid(GLuint internalformat,Mesh::IMesh &sourceMesh) : camera_(0,1,1,0,-1,1), internalformat_(internalformat),vertsperedge_(0), 
	...
	TLMMPCPUGPUHybrid::~TLMMPCPUGPUHybrid()
	{
		DestoryResources();
	}
	...
	int TLMMPCPUGPUHybrid::CreateResources(const int vertsperedge)
	{
		...
		std::fill_n(Mv,(numVerts_),vector4(0.0,0.0,0.0,0.0));
		...
		// Due to vertex format limitations this is always 32bit per channel (o rly?)
		glGenTextures(1,&normalMap_);
		glBindTexture(GL_TEXTURE_2D, normalMap_);
		SetupRenderTarget(vertsperedge,internalformat_/*GL_RGBA32F_ARB*/);
	...
	int TLMMPCPUGPUHybrid::DestoryResources() 
	{
	...
	int TLMMPCPUGPUHybrid::GetVRAMUsage()
	{
		return 0;
	}
	std::string TLMMPCPUGPUHybrid::GetFriendlyName( )
	{
		return std::string("Multi-Threaded CPU GPU Hybrid method" );
	}
	...
	void TLMMPCPUGPUHybrid::GenerateTLMData()
	{
		// Scatter
#pragma omp parallel
		{
...
#pragma omp for
	...
	int TLMMPCPUGPUHybrid::RenderTLM(const vector3& pCamera, const vector3& pLight )
	{
	...
	void TLMMPCPUGPUHybrid::DriveTLMData(float drivingValue)
	{
	...
	void TLMMPCPUGPUHybrid::VisualiseTextures()
	{
