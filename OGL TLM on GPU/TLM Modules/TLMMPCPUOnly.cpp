...
namespace TLM
{
	TLMMPCPUOnly::TLMMPCPUOnly(Mesh::IMesh &sourceMesh) : vertsperedge_(0), numVerts_(0), numIndices_(0),sourceMesh_(sourceMesh)
	{
	}

	TLMMPCPUOnly::~TLMMPCPUOnly()
	{
		DestoryResources();
	}
	...
	int TLMMPCPUOnly::CreateResources(const int vertsperedge)
	{
		...
		finalNormals_ = new vector3[numVerts_];
	...
	int TLMMPCPUOnly::DestoryResources() 
	{
		...
		delete[] finalNormals_; finalNormals_ = 0;
	...
	int TLMMPCPUOnly::GetVRAMUsage()
	{
		return 0;
	}
	std::string TLMMPCPUOnly::GetFriendlyName( )
	{
		return std::string("Multi-Thread CPU Only driven method" );
	}

	void TLMMPCPUOnly::GenerateTLMData()
	{
		// Scatter
#pragma omp parallel
		{
...
#pragma omp for
				for(int x = 0; x < vertsperedge_; x++)
				{
					...
					for(int y = 0; y < vertsperedge_; y++)
					{
						...
						finalNormals_[x + (y*vertsperedge_)] = finalNormal;
	...
	int TLMMPCPUOnly::RenderTLM(const vector3& pCamera, const vector3& pLight )
	{
	...
	void TLMMPCPUOnly::DriveTLMData(float drivingValue)
	{
	...
	void TLMMPCPUOnly::VisualiseTextures()
	{
